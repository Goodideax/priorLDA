...
int model::find(int topic, int *arr)
{
	int i;
	int cnt = 0;
	for (i = 0; i < K; i++) {
		if ((arr[i] & 0x3ff) == topic)
			cnt = arr[i] >> 10;
		if (arr[i] == 0) break;
	}
	return cnt;
...
// The inc function is used to increase (topic,word)
// entry inside the NW matrix. If the topic is existed
// , increase the entry by one. If the topic doesn't exist
// create a new entry and assign the count as 1.
void model::inc(int topic, int word, int **nw_arr)
{
	int *arr;
	arr = *nw_arr;
	int i;
	...
	/*	if(i==newCount[word]-2 && add)
		{

			int * tmp_arr = new int[newCount[word]+5];
			memset(tmp_arr, 0, sizeof(int)*(newCount[word]+5));
			memcpy(tmp_arr, arr, sizeof(int)*(i+1));//Careful, here should be i+1.
			delete[] *nw_arr;
			(*nw_arr) = tmp_arr;
			newCount[word] += 5;
			bool error = 0;
...
void model::dec(int topic, int word, int *arr)
{
	if (topic >= K)
...
// The init_est_disk_sample can be divided into three parts.
...
int model::init_est_disk_sample() {
	...
	int a;
	nw = new int*[V];
	//newCount.resize(V, K + 3);
	wordCount = ptrndata->idCount;
	realCount.resize(V, 0);
	for (w = 0; w < V; w++) {
		if (wordCount[w] <= K)
			//	nw[w].reserve(wordCount[w]);
			nw[w] = new int[wordCount[w]];
		else
			//	nw[w].reserve(K);
			nw[w] = new int[K];
...
//    int tmp = ptrndata->doc(m)->length;
	...
	{
		int len = wordCount[w];
		for (t = 0; t < min(len, K); t++)
			if (nw[w][t] == 0) break;
		count += t;
	}
	cout << "after prior learning, average topic is " << (count + 0.0) / V << endl;
	/*
	    int cnt;
	    int *tmp_arr;
	    //int t;
	    //mallopt(M_TRIM_THRESHOLD, count*4+8);
	    for(w=0; w<V; w++) {
		int len = wordCount[w];
		for(t=0;t<min(len,K);t++)
			if(nw[w][t]==0) break;
	//        t = min(len, K);
		if(t!= realCount[w])
			cout<<"mismatch"<<endl;
		tmp_arr = new int[t+5];
		memset(tmp_arr,0,sizeof(int)*(t+5));
		memcpy(tmp_arr, nw[w], sizeof(int)*(t));
		delete[] nw[w];
		nw[w] = tmp_arr;
		for(int ii=t; ii<t+5;ii++)
			if(nw[w][ii]!=0)
				cout<<"error in init"<<endl;
		newCount[w] = t+5;// newCount record the actual capacity of nw
	//	wordCount[w] = t;
	    }
	    malloc_trim(0);
	*/
	...
	for (m = 0; m < ptrndata->M; m++) {
		...
		for ( int i = 0; i < ptrndata->doc(m)->length; i++)
		{
			...
			{
				tmpcur += (nw[w][j] >> 10);
				if (tmpcur >= rand || nw[w][j] == 0) break;
			}
			if (tmpcur < rand) {
				cout << "error in 1207" << endl;
				cout << tmpsum << ' ' << rand << ' ' << tmpcur << endl;
				j = min(len, K) - 1;
			}
			tmpnd[nw[w][j] & 0x3ff]++;
		}
...
//		cout<<i<<' '<<ndsum[i]<<endl;
		...
		for ( int v = 0; v < V; v++)
		{
			...
			int len = wordCount[v];
			for ( int k = 0; k < min(K, len); k++) {
				if (nw[v][k] == 0) break;
				nw_cnt += (nw[v][k] >> 10);
				topic[(nw[v][k] & 0x3ff)] = (nw[v][k] >> 10) ;
			}
			fout << topic[0];
			for (int k = 1; k < K; k++)
				fout << ' ' << topic[k];
...
//	cout<<rsum<<endl;
		//if(newCount[w]==5 && topic != 0) cout<<"error in topic!"<<endl;
		dec(topic - 1, w, nw[w]);
		ND.visit(m, topic - 1) -= 1;
		nwsum[topic - 1] -= 1;
		ndsum[m] -= 1;
